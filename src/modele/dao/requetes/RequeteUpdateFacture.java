...
public class RequeteUpdateFacture extends Requete<Facture> {
	...
	/**
	 * Retourne la requête SQL UPDATE pour mettre à jour une facture.
	 */
	@Override
	public String requete() {
		return "Update MSF5131A.SAE_Facture " + "SET Montant = ?, " + "Date_de_facture = ?, Compte_Bancaire = ?, "
				+ "Montant_Devis = ?, Date_Paiement = ?, " + "Designation_de_travaux = ? "
	...
	/**
	 * Méthode parametres pour un objet Facture. Remplit le PreparedStatement avec
	 * les valeurs provenant de l'objet Facture.
	 * 
	 * @param prSt   PreparedStatement sur lequel on définit les paramètres
	 * @param donnee Objet Facture contenant les valeurs à mettre à jour
	 */
	@Override
