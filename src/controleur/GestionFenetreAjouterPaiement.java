...
/**
 * Contrôleur de la fenêtre d'ajout d'un paiement. Gère la création d'un
 * paiement et la mise à jour de la fenêtre parente.
 */
public class GestionFenetreAjouterPaiement extends GestionButtonFenetreAjouter {
	 ...
	 *
	 * @param fenetre              fenêtre d'ajout de paiement
	 * @param parent               contrôleur parent (liste des paiements)
	 * @param locataireSelectionne locataire actuellement sélectionné
	 */
	public GestionFenetreAjouterPaiement(FenetreAjouterPaiement fenetre, GestionFenetrePaiement parent,
			Locataire locataireSelectionne) {
	...
	/**
	 * Retourne les champs texte de la fenêtre paiement.
	 */
	@Override
	...
	/**
	 * Action exécutée lors du clic sur le bouton "Ajouter". Crée un paiement et met
	 * à jour la fenêtre parente.
	 */
	@Override
	protected void gererAction() {
		try {
			...
			Paiement p = new Paiement(donnees.get(0).getText(), Double.parseDouble(donnees.get(1).getText()),
					Date.valueOf(donnees.get(2).getText()), donnees.get(3).getText(),
					(ContratLocation) comboBox.getSelectedItem());

			if (dao.create(p) == 1) {
				JOptionPane.showMessageDialog(null, "Paiement ajoutée avec succès !", "Succès",
				...
				JOptionPane.showMessageDialog(null, "Échec de l'ajout du paiement.", "Erreur",
			...
			e.printStackTrace();
			if (e.getErrorCode() == 1) {
				JOptionPane.showMessageDialog(null, "Ce paiement existe déjà (clé primaire).", "Doublon",
	...
	/**
	 * Charge les contrats de location dans la ComboBox. Sélectionne automatiquement
	 * le contrat du locataire si disponible.
	 */
