...
public class GestionFenetreAjouterCharge extends GestionButtonFenetreAjouter {
	...
	protected void gererAction() {
		try {
			...
			ChargesGenerales cg = new ChargesGenerales(donnees.get(0).getText(), donnees.get(1).getText(),
					Double.parseDouble(donnees.get(2).getText()), Float.parseFloat(donnees.get(3).getText()),
					Double.parseDouble(donnees.get(4).getText()), Date.valueOf(donnees.get(5).getText()), b);
			if (dao.create(cg) == 1) {
				JOptionPane.showMessageDialog(null, "Charge ajoutée avec succès !", "Succès",
						JOptionPane.INFORMATION_MESSAGE);
				this.parent.setDonnees(dao.findByIdBien(b.getIdBienLouable()));
				this.parent.chargerDonnees();
				this.fenetre.dispose();

			} else {
				JOptionPane.showMessageDialog(null, "Échec de l'ajout de la charge.", "Erreur",
