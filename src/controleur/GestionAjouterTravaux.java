...
public class GestionAjouterTravaux extends GestionButtonFenetreAjouter {
	...
	public GestionAjouterTravaux(FenetreAjouterTravaux fenetreAjouterTravaux, GestionFenetreTravaux parent,
			BienLouable bl) {
	...
	/**
	 * Gère l'action de création d'une facture/travaux. - Récupère les données
	 * saisies dans les champs texte et combo box - Crée un objet Facture - Tente
	 * l'insertion en base via DaoFacture - Affiche un message de succès ou d'erreur
	...
	protected void gererAction() {
		try {
			...
			Facture f = new Facture(donnees.get(0).getText(), Double.parseDouble(donnees.get(1).getText()),
					Date.valueOf(donnees.get(2).getText()), donnees.get(3).getText(),
					Double.parseDouble(donnees.get(4).getText()), Date.valueOf(donnees.get(5).getText()),
					donnees.get(6).getText(), bl, (Entreprise) comboBox.getSelectedItem());
			...
			if (daoFac.create(f) == 1) {
				...
				JOptionPane.showMessageDialog(null, "Échec de l'ajout de la facture.", "Erreur",
			...
			e.printStackTrace();
			if (e.getErrorCode() == 1) {
				JOptionPane.showMessageDialog(null, "Ce travaux existe déjà (clé primaire).", "Doublon",
