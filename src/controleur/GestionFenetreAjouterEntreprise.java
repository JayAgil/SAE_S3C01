...
public class GestionFenetreAjouterEntreprise extends GestionButtonFenetreAjouter {
	...
	/**
	 * Récupère tous les champs texte de la fenêtre pour l'entreprise.
	 */
	@Override
	 ...
	 * crée une nouvelle entreprise et ferme la fenêtre si succès.
	 */
	@Override
	protected void gererAction() {
		try {
			...
			Entreprise e = new Entreprise(donnees.get(0).getText(), donnees.get(1).getText(), donnees.get(2).getText(),
					donnees.get(3).getText(), donnees.get(4).getText(), donnees.get(5).getText(),
					donnees.get(6).getText());
			if (dao.create(e) == 1) {
				JOptionPane.showMessageDialog(null, "Entreprise ajoutée avec succès !", "Succès",
						JOptionPane.INFORMATION_MESSAGE);
				this.fenetre.dispose();

			} else {
				JOptionPane.showMessageDialog(null, "Échec de l'ajout de l'entreprise.", "Erreur",
			...
			e.printStackTrace();
			if (e.getErrorCode() == 1) {
				JOptionPane.showMessageDialog(null, "Cette entreprise existe déjà (clé primaire).", "Doublon",
